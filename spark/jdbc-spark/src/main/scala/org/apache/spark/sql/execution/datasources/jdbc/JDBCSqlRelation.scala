...
/**
  ...
  */
private[sql] case class JDBCSqlPartitioningInfo(
                                                 column: String,
                                                 lowerBound: Long,
                                                 upperBound: Long,
                                                 numPartitions: Int)

private[sql] object JDBCSqlRelation extends Logging {
  /**
    ...
    */
  def columnPartition(partitioning: JDBCSqlPartitioningInfo): Array[Partition] = {
    if (partitioning == null || partitioning.numPartitions <= 1 ||
      partitioning.lowerBound == partitioning.upperBound) {
      return Array[Partition](JDBCSqlPartition(null, 0))
    }
    ...
    while (i < numPartitions) {
      ...
      ans += JDBCSqlPartition(whereClause, i)
      i = i + 1
    }
    ans.toArray
  }
...
private[sql] case class JDBCSqlRelation(
  ...
  override val schema: StructType = JDBSqlCRDD.resolveTable(jdbcOptions)

  // Check if JDBCRDD.compileFilter can accept input filters
  override def unhandledFilters(filters: Array[Filter]): Array[Filter] = {
    filters.filter(JDBSqlCRDD.compileFilter(_, JdbcDialects.get(jdbcOptions.url)).isEmpty)
  }

  override def buildScan(requiredColumns: Array[String], filters: Array[Filter]): RDD[Row] = {
    // Rely on a type erasure hack to pass RDD[InternalRow] back as RDD[Row]
    JDBSqlCRDD.scanTable(
  ...
  override def toString: String = {
    ...
    s"JDBCSqlRelation(${jdbcOptions.table})" + partitioningInfo
  }
}
