...
/**
  ...
  */
case class JDBCSqlPartition(whereClause: String, idx: Int) extends Partition {
  override def index: Int = idx
}

object JDBSqlCRDD extends Logging {
  ...
  def resolveTable(options: JDBCOptions): StructType = {
    ...
    val conn: Connection = JdbcSqlUtils.createConnectionFactory(options)()
    try {
      val statement = conn.prepareStatement(dialect.getSchemaQuery(table))
      try {
        val rs = statement.executeQuery()
        try {
          JdbcSqlUtils.getSchema(rs, dialect)
        } finally {
          rs.close()
        }
      } finally {
  ...
  def scanTable(
    ...
    new JDBSqlCRDD(
      sc,
      JdbcSqlUtils.createConnectionFactory(options),
...
private[jdbc] class JDBSqlCRDD(
  ...
  private val filterWhereClause: String =
    filters
      .flatMap(JDBSqlCRDD.compileFilter(_, JdbcDialects.get(url)))
      .map(p => s"($p)").mkString(" AND ")

  /**
    * A WHERE clause representing both `filters`, if any, and the current partition.
    */
  private def getWhereClause(part: JDBCSqlPartition): String = {
  ...
  override def compute(thePart: Partition, context: TaskContext): Iterator[InternalRow] = {
    ...
    val part = thePart.asInstanceOf[JDBCSqlPartition]
    conn = getConnection()
    val dialect = JdbcDialects.get(url)
    ...
    val sqlText = s"${selectClause} $myWhereClause"
    stmt = conn.prepareStatement(sqlText,
      ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY)
    stmt.setFetchSize(options.fetchSize)
    rs = stmt.executeQuery()
    val rowsIterator = JdbcSqlUtils.resultSetToSparkInternalRows(rs, schema, inputMetrics)
